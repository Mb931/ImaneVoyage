...
$nom = $_POST["nom"];
$prenom = $_POST["prenom"];
$authentification_motdepasse=$_POST["authentification_motdepasse"];
$email=$_POST["email"];
// Vérification des champs nom et prenom (si il ne sont pas vides ?)
if( empty($nom) || empty($prenom) || empty($authentification_motdepasse)  || empty($email))  // le signe || signifie OU
{
...
else // $nom et $prenom sont corrects  
{
	...
	$nb_lignes=insert_authentification($nom, $prenom,$authentification_motdepasse,$email); // appel de fonction d'insertion (couche Modele)
